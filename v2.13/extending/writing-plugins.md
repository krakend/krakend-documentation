...
A **simplified version** of the [Execution Flow](/docs/v2.13/design/execution-flow/#execution-flow-pipes) is:
...
| -------- | ------- | ------- |
| {{< badge color="#6f00f0">}}server{{< /badge >}}      | Router | [HTTP server plugins](/docs/v2.13/extending/http-server-plugins/) modify the HTTP request and response between the end-user and KrakenD (the server)    |
| {{< badge color="#0000ff">}}req/resp{{< /badge >}}    | Proxy | [Request/Response Modifier plugins](/docs/v2.13/extending/plugin-modifiers/) modify data (such as headers, body, status code...) |
...
| {{< badge color="#e900b7">}}client{{< /badge >}}      | Transport | [HTTP client plugins](/docs/v2.13/extending/http-client-plugins/) modify the request and response between KrakenD and the upstream services (internal HTTP client)    |
...
Before compiling the plugin, you must ensure the libraries you use in your code are compatible with KrakenD. To do so, execute the command [`{{< product check_plugin_command >}}`](/docs/v2.13/extending/check-plugin/) that analyzes your `go.sum` file and warns you about incompatibilities.
...
|--------------|---------------------------------------------------------------------|-----------------------------------------------------------------------------------|
| [AMD64](/docs/v2.13/extending/writing-plugins/#compile-plugins-for-amd64)        | `{{< product image_plugin_builder >}}:2.13` | `{{< product image_plugin_builder >}}:2.13-linux-generic` |
| [ARM64](/docs/v2.13/extending/writing-plugins/#compile-plugins-for-arm64)        | `{{< product image_plugin_builder >}}:2.13` with cross-compile instructions | `{{< product image_plugin_builder >}}:2.13-linux-generic` with cross-compile instructions |
...
docker run -it -v "$PWD:/app" -w /app {{< product image_plugin_builder >}}:2.13 go build -buildmode=plugin -o yourplugin.so .
{{< /terminal >}}

The command will generate a `yourplugin.so` file (name it as you please) that you can now copy into a `{{< product image >}}:2.13` Docker image, and load it as described in [injecting plugins](/docs/v2.13/extending/injecting-plugins/). **Never use a tag that mismatches the builder and KrakenD**. If you want to load the plugin in a KrakenD version `x.y.x` make sure to build it on a builder `x.y.z`. Using `.so` files that were compiled in a builder with a different version, will mostly fail.
...
docker run -it -v "$PWD:/app" -w /app {{< product image_plugin_builder >}}:2.13-linux-generic go build -buildmode=plugin -o yourplugin.so .
{{< /terminal >}}
...
-e "GOARCH=arm64" \
-e "GOHOSTARCH=amd64" \
{{< product image_plugin_builder >}}:2.13 \
...
-e "GOARCH=arm64" \
-e "GOHOSTARCH=amd64" \
{{< product image_plugin_builder >}}:2.13-linux-generic \
...
## Test the plugin
Once your `.so` file is available, you must check that the plugin is loadable by KrakenD. You can test the plugin using the [test command](/docs/v2.13/extending/test-plugin/)
...
See the detailed information to [inject plugins](/docs/v2.13/extending/injecting-plugins/))
