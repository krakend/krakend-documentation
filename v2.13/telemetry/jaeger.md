...
To add Jaeger, configure a new exporter to the [OpenTelemetry settings](/docs/v2.13/telemetry/opentelemetry/). For instance:
...
{{< schema version="v2.13" data="telemetry/opentelemetry.json" property="exporters" filter="otlp">}}

But as you can see there is a `layers` attribute in the example configuration that defines settings for all exporters (not only Jaeger). See the [layers options](/docs/v2.13/telemetry/opentelemetry/#layers).
