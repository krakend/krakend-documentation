...
](/docs/enterprise/service-settings/service-rate-limit/) or [endpoint](/docs/v2.13/endpoints/rate-limit/) types) is the recommended approach for almost all scenarios. As the API Gateway does not have any centralization, **the limits apply individually to each running instance of KrakenD**.
...
When you have to do small math like this, through the [Flexible Configuration](/docs/v2.13/configuration/flexible-config/), you might inject environment variables when starting KrakenD with the total number of machines you have. For instance:
