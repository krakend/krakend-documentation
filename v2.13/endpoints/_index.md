...
If you are still getting familiar with KrakenD's configuration structure, take a moment to read [Understanding the configuration file](/docs/v2.13/configuration/structure/). This section explains what an `endpoint` looks like, but take a moment to understand the different pieces.
{{< /note >}}
...
{{< schema version="v2.13" data="endpoint.json" >}}
...
By default KrakenD only works with **RESTful URL patterns** in its endpoint definition. Enable the option [`disable_rest`](/docs/v2.13/service-settings/http-server-settings/#disable_rest) in the root of your configuration if need unrestful endpoints, e.g.: `/file.{extension}`
...
The endpoints return HTTP content to the end-user in any of the [supported encodings](/docs/v2.13/endpoints/content-types/), regardless of the type of backend you are connecting to.
...
{{< schema version="v2.13" data="endpoint_extra_config.json" norecurse="auth/api-keys,proxy" title="Available components of an endpoint">}}
