...
Also worth mentioning, the operator works with **objects only** and not with arrays. When working with collections `[]`, see the special case [manipulating arrays](/docs/v1.4/backends/flatmap/).

For instance, `{ "a": [ { "a1": 1 } ] }` can't be denied as `a.a1` because `a1` is inside an array.

{{< note title="Working with arrays" >}}
Deny list and allow list filters expect objects. When working with collections, see the [flatmap approach](/docs/v1.4/backends/flatmap/)
{{< /note >}}
...
See [Manipulating arrays - flatmap](/docs/v1.4/backends/flatmap/)

## Manipulations with Lua
If you need more sophisticated manipulation options, they can be done using Lua scripting:

See [Lua scripts](/docs/v1.4/endpoints/lua/) documentation
