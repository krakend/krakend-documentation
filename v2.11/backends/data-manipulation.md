...
---
This page describes the most basic options to manipulate the content you receive from the `backend` before delivering it to the endpoint to [aggregate data from all backends](/docs/v2.11/endpoints/response-manipulation/#aggregation-and-merging).
...
Also worth mentioning, the operator works with **objects only** and not with arrays. When working with collections `[]`, see the special case [manipulating arrays](/docs/v2.11/backends/flatmap/).
...
Deny list and allow list filters expect objects. When working with collections, see the [flatmap approach](/docs/v2.11/backends/flatmap/)
{{< /note >}}
...
There will be times when you want to move one item to another place, or flatten its structure. In these cases you need to use the [flatmap component](/docs/v2.11/backends/flatmap/).
...
```
See how the [flatmap filter](/docs/v2.11/backends/flatmap/) works for more options.
...
The use of `is_collection` can be avoided when the backend uses as `encoding` the value `safejson`. See [supported encodings](/docs/v2.11/backends/supported-encodings/)
{{< /note >}}
...
See [Manipulating arrays - flatmap](/docs/v2.11/backends/flatmap/)
...
- Through [Response modifier plugins](/docs/v2.11/extending/plugin-modifiers/) - Very performant, requires compilation
- Through [Lua scripting](/docs/v2.11/endpoints/lua/) - Less performant, does not require compilation
