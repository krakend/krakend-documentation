...
2. **Adjust your configuration and run the linter** as needed. Below, you'll find the changes between versions. Scroll down to your current version and apply all changes above it. Then, run [the linter](/docs/v2.11/configuration/check/) (`krakend check --lint`), which is designed to scrutinize your configuration rigorously.
...
4. If you have custom plugins, you need to recompile them with the enterprise builder, and rename in your CI/CD the [test](/docs/v2.11/extending/test-plugin/) and [check](/docs/v2.11/extending/check-plugin/) plugin commands.
...
**If you have custom go plugins**, recompile them. KrakenD has now a command [`krakend check-plugin`](/docs/v2.11/extending/check-plugin/) and [`krakend test-plugin`](/docs/v2.11/extending/test-plugin/) to test them.
