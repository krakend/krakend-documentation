...
The [OpenTelemetry integration](/docs/v2.11/v2.10/telemetry/opentelemetry/) allows you to send **metrics and traces** to Datadog using their collector.
...
In addition, you can configure how the `layers` behave ([see all options](/docs/v2.11/v2.10/telemetry/opentelemetry/#layers)).
...
Prior to v2.6, telemetry sent to Datadog used the OpenCensus exporter. Enabling required adding the `datadog` exporter in the [opencensus module](/docs/v2.11/v2.10/telemetry/opencensus/), and the configurations looked like this:
```json
{
      "version": 3,
      "extra_config": {
