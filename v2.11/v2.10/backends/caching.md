...
The components [Client Credentials](/docs/v2.11/v2.10/authorization/client-credentials/), [Lambda functions](/docs/v2.11/v2.10/backends/lambda/), [AMQP Consumers or Producers](/docs/v2.11/v2.10/backends/amqp-consumer/), [Publish/Subscribe](/docs/v2.11/v2.10/backends/pubsub/), and [HTTP Client plugins](/docs/v2.11/v2.10/extending/http-client-plugins/) **don't support direct caching** because they use a **custom HTTP client** with extended functionality that does not use the one with caching capabilities. Generally speaking, connections with upstream services that need authentication or custom HTTP clients are not eligible to cache their responses.
{{< /note >}}
...
The [Martian module](/docs/v2.11/v2.10/backends/martian/) is the component that can transform the headers from the backend as long as you don't use the `no-op` encoding (as it does not allow manipulation). Let's illustrate how you can do this in the following example.
