...
All the configuration inside the namespace is pure JSON Schema vocabulary. [Read the JSON schema documentation](https://json-schema.org/) to get familiar with the specification.
 ...
 A full configuration for you to try on the localhost with the [debug endpoint](/docs/v2.11/v2.10/endpoints/debug-endpoint/) is:
...
Still, you can show the **JSON schema error message** to the end user by [enabling the `return_error_msg`](/docs/v2.11/v2.10/service-settings/router-options/#return_error_msg) in the router options.
