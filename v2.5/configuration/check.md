...
The `krakend check` command **validates KrakenD configuration files** written in any of its [supported formats](/docs/v2.5/configuration/supported-formats/).
...
- `-c` or `--config` to specify the path to the configuration file in any of the [supported formats](/docs/v2.5/configuration/supported-formats/), or to the starting template if used in combination with flexible configuration.
...
- `-l` or `--lint` (*optional*) to check that your configuration file is properly linted and does not contain unrecognized options or wrong types. This option requires Internet access as the schema is downloaded from `https://www.krakend.io/schema/v2.5/krakend.json` (each version uses its schema).
