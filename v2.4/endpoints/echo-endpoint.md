...
notoc: true
---
The `/__echo/` endpoint is a developer tool to help you debug configurations. It works similarly to the [`/__debug/` endpoint](/docs/v2.4/endpoints/debug-endpoint/), but instead of printing the requests in the log and returning a `{"message": "pong"}`, they are printed in the response. It replies with an object containing all the request details, and you can use it as an endpoint or backend. As KrakenD has a [zero-trust](/docs/v2.4/design/zero-trust/) approach, you will find out the exact information that passes through in this endpoint.
...
{{< schema version="v2.4" data="krakend.json" filter="echo_endpoint">}}
...
Play now with the [parameter forwarding](/docs/v2.4/endpoints/parameter-forwarding/), such as the `input_headers` and `input_query_strings`, and repeat the test.