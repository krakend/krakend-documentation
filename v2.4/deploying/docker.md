...
A more real-life example illustrates below a combination of the `check` command with a multi-stage build to compile a [flexible configuration](/docs/v2.4/configuration/flexible-config/) in a `Dockerfile`:

```docker
FROM {{< product image >}}:2.4 as builder
ARG ENV=prod
