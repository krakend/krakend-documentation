...
To create Async agents that consume messages asynchronously and without requiring a user request, see [Async Agents](/docs/v2.9/async/).
...
See [Async Agents](/docs/v2.9/async/) to consume messages without an endpoint.
...
{{< schema version="v2.9" data="backend/amqp/consumer.json" >}}

## Connection retries
The gateway will try to reconnect to the AMQP server if the connection is lost for any reason. You can set the [back-off strategy](/docs/v2.9/async/#backoff-strategies) that better fits your needs.
