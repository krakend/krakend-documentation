...
To add Jaeger, configure a new exporter to the [OpenTelemetry settings](/docs/v2.12/telemetry/opentelemetry/). For instance:
...
But as you can see there is a `layers` attribute in the example configuration that defines settings for all exporters (not only Jaeger). See the [layers options](/docs/v2.12/telemetry/opentelemetry/#layers).
