...
OpenTelemetry captures detailed, contextual information about the operation of your applications. This includes not only **metrics** but also **tracing data** that shows the full lifecycle of requests as they flow through your systems, providing insights into performance bottlenecks, latency issues, and error diagnostics. This OpenTelemetry **does not** support pushing logs. See [logging](/docs/v2.12/logging/), [GELF](/docs/v2.12/logging/graylog-gelf/), or [Logstash](/docs/v2.12/logging/logstash/) instead.
...
KrakenD has traditionally offered part of its telemetry integration through the [OpenCensus integration](/docs/v2.12/telemetry/opencensus/), which has provided a reliable service for over six years. We are transitioning to the more modern and robust OpenTelemetry framework, and the OpenCensus integration does not receive further updates.
...
When you add OpenTelemetry in the configuration, you will have [different metrics available](/docs/v2.12/telemetry/opentelemetry-layers-metrics/).
...
[See how to configure Prometheus](/docs/v2.12/telemetry/prometheus/)

## OTLP exporter (push)
Choose the `otlp` exporter when you want to **push the metrics to a local or remote collector** or directly to a SaaS or storage system that supports native OTLP (there is a [large number of supported providers](/docs/v2.12/telemetry/#opentelemetry-integrations)). The following diagram represents this idea:
...
{{< schema version="v2.12" data="telemetry/opentelemetry.json" >}}
...
- Push metrics to [InfluxDB](/docs/v2.12/telemetry/influxdb/)
- Pull metrics from [Prometheus](/docs/v2.12/telemetry/prometheus/)
- Push metrics to [Datadog](/docs/v2.12/telemetry/datadog/)
- Push metrics to [Zipkin](/docs/v2.12/telemetry/zipkin/)
- Push metrics to [Jaeger](/docs/v2.12/telemetry/jaeger/)
- Push metrics to [Azure Monitor](/docs/v2.12/telemetry/azure/)