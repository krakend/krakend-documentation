...
Scripting with Lua allows you to extend your business logic and make **transformations on requests and responses**. The Lua module is compatible with the rest of components such as [CEL](/docs/v2.12/endpoints/common-expression-language-cel/), [Martian](/docs/v2.12/backends/martian/), or other [Go plugins](/docs/v2.12/extending/) and middlewares.
...
A [Go plugin](/docs/v2.12/extending/) delivers much more speed and power than a Lua script for performance-first seeking users, but requires a little bit more work as you need to compile your plugins and side-load them on KrakenD.
{{< /note >}}
...
{{< schema version="v2.12" data="modifier/lua.json" >}}
...
When **client headers** or **query strings** are needed in a script, remember to add them under [`input_headers`](/docs/v2.12/endpoints/parameter-forwarding/#headers-forwarding) or [`input_query_strings`](/docs/v2.12/endpoints/parameter-forwarding/#query-string-forwarding) accordingly.
{{< /note >}}
