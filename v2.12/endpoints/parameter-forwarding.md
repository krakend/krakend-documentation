...
{{< schema version="v2.12" data="endpoint.json" filter="input_headers,input_query_strings" >}}
...
By definition, query string parameters are always optional, and the user can pass a subset of them, all or none. Suppose you want to enforce that the user provides a query string parameter. In that case, you must validate it with the [Common Expression Language](/docs/v2.12/endpoints/common-expression-language-cel/) (faster) or with a [Lua script](/docs/v2.12/endpoints/lua/) (slower).
...
The `X-Forwarded`-like headers are controlled by [`forwarded_by_client_ip`](/docs/v2.12/service-settings/router-options/#forwarded_by_client_ip). All these headers **are always sent, regardless of whether you define them or not** under `input_headers`. You can override their values.
...
- `User-Agent` Contains the value `KrakenD Version 2.12`, unless you add it to the `input_headers`
...
In addition, there are a few KrakenD components that support **setting attributes in headers**, like for instance [`propagate_claims` in JWT](/docs/v2.12/authorization/jwt-validation/#propagate_claims). These components will send the additional headers you configure to the backend automatically.
...
Read the [`/__debug/` endpoint](/docs/v2.12/endpoints/debug-endpoint/) to understand how to test headers.
...
If you'd like to take those headers into account, use the flag [`forwarded_by_client_ip` under the router section](/docs/v2.12/service-settings/router-options/#forwarded_by_client_ip)
{{< /note >}}
