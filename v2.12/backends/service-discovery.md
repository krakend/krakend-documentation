...
You might also want to read the [Load Balancer](/docs/v2.12/throttling/load-balancing/) documentation
{{< /note >}}
...
KrakenD must be in a network that can reach any declared hosts. With more than one host, KrakenD [load balances](/docs/v2.12/throttling/load-balancing/) the connections to the hosts in the list.

## Service discovery configuration
The possible configurations and values for `sd` are:

{{< schema version="v2.12" data="backend.json" filter="sd,sd_scheme" >}}
...
{{< schema version="v2.12" data="_root.json" filter="dns_cache_ttl" title="Override DNS cache" >}}
