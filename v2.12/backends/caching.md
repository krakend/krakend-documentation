...
{{< schema version="v2.12" data="backend_extra_config.json" filter="qos/http-cache" title="Cache options">}}
...
The [Martian module](/docs/v2.12/backends/martian/) is the component that can transform the headers from the backend as long as you don't use the `no-op` encoding (as it does not allow manipulation). Let's illustrate how you can do this in the following example.
