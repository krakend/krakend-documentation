...
- The token is not revoked in the bloom filter (see [revoking tokens](/docs/v2.2/authorization/revoking-tokens/))
...
These options are for the `extra_config`'s namespace `"auth/validator"` placed in every endpoint (use [flexible configuration](/docs/v2.2/configuration/flexible-config/) to avoid code repetition):

{{< schema version="v2.2" data="auth/validator.json" >}}
