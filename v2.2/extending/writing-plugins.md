...
Before getting your hands dirty, read the [introduction to plugins](/docs/v2.2/extending/) to understand **the different plugins** you can use and choose the one that best adapts to your needs.
{{< /note >}}
...
- The [command `check-plugin`](/docs/v2.2/extending/check-plugin/) analyzes your `go.sum` file and warns you about incompatibilities.
...
docker pull {{< product image_plugin_builder >}}:2.2
{{< /terminal >}}
...
docker run -it -v "$PWD:/app" -w /app {{< product image_plugin_builder >}}:2.2 go build -buildmode=plugin -o yourplugin.so .
{{< /terminal >}}

The command will generate a `yourplugin.so` file (name it as you please) that you can now copy into a `{{< product image >}}:2.2` Docker image (but not to tag mismatching the builder), and load it as described in [injecting plugins](/docs/v2.2/extending/injecting-plugins/).
...
As your custom plugins need to match the Go and libraries versions used to build KrakenD, you have to guarantee your plugin is compatible by checking the `go.sum` file with the command `check-plugin` ([read the documentation](/docs/v2.2/extending/check-plugin/))
...
Now load it in KrakenD, as described in [injecting plugins](/docs/v2.2/extending/injecting-plugins/)
