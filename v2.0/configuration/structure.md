...
weight: 10
---
All KrakenD behavior depends on its configuration file(s). Although the configuration [supports formats other than JSON](/docs/v2.0/configuration/supported-formats/) and it can be described [using multiple files](/docs/v2.0/configuration/flexible-config/), you'll find it referenced through all this documentation and for simplicity as the `krakend.json`. Being familiar with its structure it's essential.
...
That's the basic structure of endpoints; for more information see [how to create endpoints](/docs/v2.0/endpoints/).
...
For instance, the [extended logging component](/docs/v2.0/logging/) uses the **namespace** `telemetry/logging`:
