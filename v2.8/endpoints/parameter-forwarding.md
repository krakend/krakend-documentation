...
---
KrakenD is an API Gateway with a **[zero-trust security policy](/docs/v2.8/design/zero-trust/)**, and when it comes to forward query strings, cookies, and headers, you need to define what is allowed.
...
{{< schema version="v2.8" data="endpoint.json" filter="input_headers,input_query_strings" >}}
...
By definition, query string parameters are always optional, and the user can pass a subset of them, all or none. Suppose you want to enforce that the user provides a query string parameter. In that case, you must validate it with the [Common Expression Language](/docs/v2.8/endpoints/common-expression-language-cel/) (faster) or with a [Lua script](/docs/v2.8/endpoints/lua/) (slower).
...
- `Accept-Encoding`
- `Host`
- `User-Agent` (KrakenD Version 2.8)
- `X-Forwarded-For`
- `X-Forwarded-Host`
- `X-Forwarded-Via` (only when `User-Agent` is in the `input_headers`)

Except for the `X-Forwarded`-like headers that are controlled by [`forwarded_by_client_ip`](/docs/v2.8/service-settings/router-options/#forwarded_by_client_ip), you can override the rest when present in the `input_headers`.

There are a few KrakenD components that support **setting attributes in headers**, like for instance [`propagate_claims` in JWT](/docs/v2.8/authorization/jwt-validation/#propagate_claims). These components will send the additional headers you configure to the backend automatically.
...
Read the [`/__debug/` endpoint](/docs/v2.8/endpoints/debug-endpoint/) to understand how to test headers.
...
If you'd like to take those headers into account, use the flag [`forwarded_by_client_ip` under the router section](/docs/v2.8/service-settings/router-options/#forwarded_by_client_ip)
{{< /note >}}
