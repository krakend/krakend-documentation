...
A more real-life example illustrates below a combination of the `check` command with a multi-stage build to compile a [flexible configuration](/docs/v2.8/configuration/flexible-config/) in a `Dockerfile`:

```docker
FROM {{< product image >}}:2.8 as builder
ARG ENV=prod
