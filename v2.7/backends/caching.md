...
When you add in a backend [Client Credentials](/docs/v2.7/authorization/client-credentials/), [Lambda functions](/docs/v2.7/backends/lambda/), [AMQP Consumers or Producers](/docs/v2.7/backends/amqp-consumer/), [Publish/Subscribe](/docs/v2.7/backends/pubsub/), or custom [HTTP Client plugins](/docs/v2.7/extending/http-client-plugins/), they overwrite the standard HTTP client and caching is disabled.
{{< /note >}}
...
The [Martian module](/docs/v2.7/backends/martian/) is the component that can transform the headers from the backend as long as you don't use the `no-op` encoding (as it does not allow manipulation). Let's illustrate how you can do this in the following example.
