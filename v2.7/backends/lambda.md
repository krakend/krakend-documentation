...
Notice in the examples that the `host` and `url_pattern` are needed as per the [backend definition](/docs/v2.7/backends/), but KrakenD will never use them when connecting to a Lambda. Feel free to add any value in there, but the entry must be present.
{{< /note >}}
...
{{< schema version="v2.7" data="backend/lambda.json" >}}
...
docker run --rm -it -p "8080:8080" \
    ...
    -v "$PWD:/etc/krakend" {{< product image >}}:2.7
{{< /terminal >}}
