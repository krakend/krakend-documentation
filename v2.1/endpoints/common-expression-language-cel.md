...
Finally, when combined with the [sequential proxy](/docs/v2.1/endpoints/sequential-proxy/), you can skip requesting a backend if a previous call didn't fulfill your criteria.
...
{{< schema version="v2.1" data="validation/cel.json" property="items" >}}
...
- The backend 0 (first item in the `backend` list) calls the URL `/__debug/0`. It returns the object `{"message": "pong"}` as per the [debug endpoint](/docs/v2.1/endpoints/debug-endpoint/) definition.
