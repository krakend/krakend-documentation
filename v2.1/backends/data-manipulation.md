...
Also worth mentioning, the operator works with **objects only** and not with arrays. When working with collections `[]`, see the special case [manipulating arrays](/docs/v2.1/backends/flatmap/).

For instance, `{ "a": [ { "a1": 1 } ] }` can't be denied as `a.a1` because `a1` is inside an array.

{{< note title="Working with arrays" >}}
Deny list and allow list filters expect objects. When working with collections, see the [flatmap approach](/docs/v2.1/backends/flatmap/)
{{< /note >}}
...
The use of `is_collection` can be avoided when the backend uses as `encoding` the value `safejson`. See [supported encodings](/docs/v2.1/backends/supported-encodings/)
{{< /note >}}
...
See [Manipulating arrays - flatmap](/docs/v2.1/backends/flatmap/)
...
- Through [Response modifier plugins](/docs/v2.1/extending/plugin-modifiers/) - Very performant, requires compilation
- Through [Lua scripting](/docs/v2.1/endpoints/lua/) - Less performant, does not require compilation
