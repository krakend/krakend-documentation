...
3) **If you don't have an identity server**, you can still use your classic monolith/backend login system and adapt it to return a JWT payload (a simple JSON). From here, let KrakenD [sign the token for you](/docs/v2.6/authorization/jwt-signing/) and start using tokens immediately.
...
6) **If you are new to JWT validation**, start reading the [JSON Web Tokens overview](/docs/v2.6/authorization/jwt-overview/)
...
The JWT validation must be present inside every endpoint definition needing it. If several endpoints are going to require JWT validation, consider using the [flexible configuration](/docs/v2.6/configuration/flexible-config/) to avoid repetitive declarations.
...
- The token is not revoked in the bloom filter (see [revoking tokens](/docs/v2.6/authorization/revoking-tokens/))
...
These options are for the `extra_config`'s namespace `"auth/validator"` placed in every endpoint (use [flexible configuration](/docs/v2.6/configuration/flexible-config/) to avoid code repetition):

{{< schema version="v2.6" data="auth/validator.json" >}}
...
### Caching keys
A crucial decision when using JWT is [defining a caching strategy](/docs/v2.6/authorization/jwk-caching/) that works with your **key rotation** policy.
...
```
[Read more about JWK caching](/docs/v2.6/authorization/jwk-caching/)

### Validation process
KrakenD does the following validation to let users hit protected endpoints:

- The `jwk_url` must be accessible by KrakenD at all times ([caching is recommended](/docs/v2.6/authorization/jwk-caching/))
...
The [KrakenD Playground](/docs/v2.6/overview/playground/) demonstrates how to protect endpoints using JWT and includes two examples ready to use:
