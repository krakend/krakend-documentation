...
[Setup a cluster of machines](/docs/v2.6/deploying/clustering/)

### Place a balancer in front of KrakenD
Put a [load balancer](/docs/v2.6/throttling/load-balancing/) in front of KrakenD to distribute traffic between the different nodes of the cluster (Kubernetes already does this for you). Use always at least two KrakenD instances for High Availability.

### Server dimensioning
Dimension KrakenD nodes according to your expected needs and throughput.

See [server requirements](/docs/v2.6/deploying/server-dimensioning/)
...
### SSL Certificates
Even that you can start KrakenD with SSL, you can add your public SSL certificate in the load balancer or PaaS and use **internal certificates**, or even no certificates at all (termination), between the [load balancer](/docs/v2.6/throttling/load-balancing/) and KrakenD.

### Prepare for failure
Add a [circuit breaker](/docs/v2.6/backends/circuit-breaker/) to your backends to prevent KrakenD from pushing to a failing system. In addition, if you know that a particular backend does not support more than a number of requests, add a maximum number of requests using the [proxy rate limit](/docs/v2.6/backends/rate-limit/).
...
Enable the [logging](/docs/v2.6/logging/) with `CRITICAL`, `ERROR`, or `WARNING` levels at most. Avoid `INFO` and `DEBUG` levels in production at all times.
...
```
If you send the logs out to an [ELK](/docs/v2.6/logging/logstash/) or a [GELF server](/docs/v2.6/logging/graylog-gelf/), use `"stdout": false` to avoid having double output.
...
- A [Grafana Dashboard](/docs/v2.6/telemetry/grafana/)
- Fueled by [InfluxDB](/docs/v2.6/telemetry/influxdb/)
- And full traces by [Jaeger](/docs/v2.6/telemetry/jaeger/)
...
Automate the go-live process through a [CI/CD pipeline](/docs/v2.6/deploying/ci-cd/) that builds and checks KrakenD configuration before deploying.
...
It is also important adding the [audit command](/docs/v2.6/configuration/audit/), excluding any rules that do not apply to you:
...
```Dockerfile
FROM {{< product image >}}:2.6
COPY krakend.json /etc/krakend/krakend.json
```

Read more on [Docker artifacts](/docs/v2.6/deploying/docker/)
...
In large organizations with several teams using a shared gateway, you can split the endpoints into groups using folders or even different repositories. With the [flexible configuration](/docs/v2.6/configuration/flexible-config/), you can have teams working in its dedicated space and aggregate all endpoints during build time without conflicts touching the same files.
