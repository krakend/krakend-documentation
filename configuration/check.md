...
- `-l` or `--lint` (*optional*) to check that your configuration file is properly linted and does not contain unrecognized options or wrong types. **The input configuration must be in JSON format**. This option requires Internet access as the schema is downloaded from `https://www.krakend.io/schema/{{< product minor_version >}}/krakend.json` (each version uses its schema).
